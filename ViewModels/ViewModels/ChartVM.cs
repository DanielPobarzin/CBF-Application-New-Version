	...
	public class ChartVM : ViewModelBase, IChartViewModel
	{
		...
		private static ConcentrationStatisticsReport GetData(DefinedFilterParameters result)
		{
			...
			for (int i = 1; i <= result.AshConcentrationEntranceMthField.Count; i++)
			{
				...
				{
					data.OutConcentration = result.AshConcentrationEntranceToFirstField * Math.Pow(result.PassageAshFirstField, i - 1);
					break;
				}
			}
		...
		{
			return dictionary.TryGetValue(key, out double value) ? value : 0;
		}
